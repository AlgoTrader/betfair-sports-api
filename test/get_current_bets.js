...
async.series({
    ...
    {
        console.log('===== Call getCurrentBets for marketId="%s" =====', marketId);
        var inv = session.getCurrentBets("U", true, "PLACED_DATE", 200, 0, false, {marketId:marketId});
        inv.execute(function(err, res) {
            console.log('action:', res.action, 'error:', err, 'duration:', res
                    .duration() / 1000);
            if (err) {
                cb("Error in getCurrentBets", null);
            }
