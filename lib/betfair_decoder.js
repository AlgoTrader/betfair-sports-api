...
function decodeNode(node) {
    ...
    case 'array':
        ...
        for ( var childIndex in children) {
            ...
            arr.push(decodeNode(children[childIndex]));
        }
        return arr;
    case 'object':
        var obj = {};
...
function detectNodeType(node) {
    var children = node.children;
    var type = node.attributes && node.attributes['xsi:type'];

    if (type && type.match(/ArrayOf/))
        return 'array';
    else if (type && type.match(/^xsd:dateTime/))
        return 'datetime';
