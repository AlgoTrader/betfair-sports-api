...
ResponseParser.prototype.parse = function(xml) {
    ...
    var parser = sax.parser(true); // true is strict
    ...
    parser.onopentag = function(node) {
        ...
        if (node.name === 'soap:Fault') {
            return {
                error : "SOAP Fault"
            };
        } else if (node.name === 'n:Result') {
            //console.log('start XML capture');
            self.capture = true;
        } else if (self.capture === true) {
            ++self.current.children;
            var xmlNode = new XmlNode(node.name, node.attributes);
