...
exports.decompressGetMarketTradedVolumeCompressed = function(marketVolume) {
    if (markVolume.length === 0) {
        return [];
    }
    ...
    // scan runners
    for(var cnt=1; cnt<sections.length; ++cnt) {
    	var runnerComponents = sections[cnt].spilt('|');
    	
    	// first component is runner Info
    	var runnerFields = runnerComponents[0].spilt('~');
    	var runner = {
