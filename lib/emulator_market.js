...
EmulatorMarket.prototype.handlePlaceBets = function(req, res, cb) {
    ...
    function placeBets() {
        ...
        var bets = betIds.map(function(id) { return self.bets[id]; } );
...
function matchBetsUsingPrices(self, betlist) {
    ...
    for(var i=0; i<betlist.length; ++i) {
        var bet = betlist[i];
        var player = self.players[bet.selectionId];
        console.log("EMU: betId=%d selId=%s type=%s", bet.betId, bet.selectionId, bet.betType);
        ...
        if(bet.betType==='B') {
            ...
            } else if((Math.abs(bestBackPrice-bet.price)<0.0001) && (1*bestBackSize>1*bet.size)) {
                bet.matchWhole(bestBackPrice);
            }
            ...
            } else if((Math.abs(bestLayPrice-bet.price)<0.0001) && (1*bestLaySize>1*bet.size)) {
                bet.matchWhole(bestLayPrice);
            }
        }
