...
EmulatorMarket.prototype.onGetCompleteMarketPricesCompressed = function(result) {
    ...
    for ( var runnerIndex in prices.runners) {
        ...
        for ( var index in selectionPrices) {
            ...
            if (backAmount>0.01 && (bestBack.price == null || price > bestBack.price)) {
                //console.log('new best back price',price,'amount',backAmount);
                bestBack.price = price;
                bestBack.amount = backAmount;
            }
            if (layAmount>0.01 && (bestLay.price == null || price < bestLay.price)) {
                //console.log('new best lay price',price,'amount',layAmount);
                bestLay.price = price;
    ...
    console.log('EMU: complete players=', self.players);
...
EmulatorMarket.prototype.handlePlaceBets = function(req, res, cb) {
    ...
    var delay = 1000 * self.inPlayDelay + 1 * transactionalDelay;
...
function matchBetsUsingPrices(self, betlist) {
    console.log('EMU: matchBetsUsingPrices');
    betlist = betlist || [];
